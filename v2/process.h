...
std::string blank_tiles = "bnk_tiles/";
...
void process(cv::Mat &image, int tlX, int tlY)
{
    ...
    if (!fs::exists(blank_tiles+std::to_string(tlX)))
    {
        fs::create_directories(blank_tiles+std::to_string(tlX));
    }
    cv::imwrite(blank_tiles+std::to_string(tlX)+"/"+std::to_string(tlY)+".png",image);
}